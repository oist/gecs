...
The motivation behind the use of terminal will be explained in [the next session](terminal.qmd)
...
An out-of-box UNIX-based OS has limited amount of tools. For this course we will use various open-source tools which need to be downloaded, installed, updated and kept track of. A package manager is a tool that helps with that. For this course we will use [`uv`](https://docs.astral.sh/uv/), and we will dive deeper into it in [the Virtual Environments session](virtual-environments.qmd).

## uv

[`uv`](https://docs.astral.sh/uv/) is a modern Python package and project manager. It helps us create isolated environments, install dependencies, lock exact package versions, and even manage Python itself when needed.

Here we will use the terminal because `uv` is typically installed from the command line rather than through a graphical installer.
...
We will use the VS Code terminal to do the installation. To open the terminal select **View** \> **Terminal** from the menu bar, or press the `` Ctrl/Cmd+` `` keyboard shortcut. A terminal window will open at the bottom.
...
To verify that `uv` got installed successfully, restart the terminal. An easy way is to close it and open a new one. Another way is to type the following for Ubuntu users:

```bash
source ~/.bashrc
```

macOS users need to type:
...
You will see a path to the `uv` executable, often something like `/Users/username/.local/bin/uv`, where `username` is your chosen username.

# Developer Platform

Your code needs to live somewhere besides your computer. Otherwise, if you lose your computer, will you be able to access your projects? For this purpose, we will use GitHub which will take a central role alongside Git during [Collaborating with Git and GitHub](collaborating.qmd).
...
Since 2023, GitHub requires all accounts to have [multi-factor authentication (2FA)](https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/about-two-factor-authentication) for extra security. Here are the recommended options:

**Recommended: Passkeys**

[Passkeys](https://docs.github.com/en/authentication/authenticating-with-a-passkey/signing-in-with-a-passkey) are the most secure and convenient option. They work on most devices and browsers without requiring a separate app.

**Other 2FA Options:**

1. **Authenticator App** (e.g., Google Authenticator, Duo Mobile) - Install on your smartphone and add GitHub
2. **Text Message (SMS)** - Receive authentication codes via text ([check if your country is supported](https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/countries-where-sms-authentication-is-supported))
3. **Hardware Security Key** - Use devices like [YubiKey](https://www.yubico.com/products/yubikey-5-overview/) or [Google Titan](https://store.google.com/us/product/titan_security_key?hl=en-US&pli=1)

For detailed setup instructions, see [GitHub's 2FA configuration guide](https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/configuring-two-factor-authentication).

### Git Identity Configuration

Before creating commits, Git needs to know who you are. Open your terminal in VS Code and set your name and email address:
...
::: {.callout-note appearance="simple"}
Make sure to use the email address associated with your GitHub account so that your commits are correctly linked to your GitHub profile.
:::

## Pre-flight Verification Check

Before attending Session 1, verify that your local Git and GitHub authentication work together with a disposable repository check:

1. In your browser on GitHub, create a new temporary public repository named `gecs-check`.
2. In VS Code, open the Command Palette (`Cmd+Shift+P` on macOS, `Ctrl+Shift+P` on Linux/WSL) and run **Git: Clone**.
3. Paste the URL of your new repository and select a local folder to save it.
4. When prompted by VS Code, sign in with your GitHub account to authorize Git.
5. In VS Code Explorer, open the cloned folder, create a new file named `test.txt`, stage it, and commit with message `test: check git setup`.
6. Click **Sync Changes** (or **Push**) to push the commit to GitHub.
7. Confirm that the commit appears on GitHub. Once confirmed, you can safely delete the `gecs-check` repository on GitHub and your local folder.

::: {.callout-tip appearance="simple"}
If your push succeeds, your environment is 100% ready for Session 1! If you encounter authentication or permission issues, bring them to the pre-course setup clinic.
:::
